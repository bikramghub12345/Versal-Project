 ...
 * Modified: AMD XRT TensorBuffer validity + FORCED page-aligned fallback
 *
 * Reference:
 *   Link 2: https://github.com/Xilinx/Vitis-AI-Tutorials/tree/3.0/Tutorials/RESNET18/
 *   Link 1: https://github.com/Xilinx/Vitis-AI-Tutorials/tree/3.0/Tutorials/Keras_FCN8_UNET_segmentation
 ...
 *
 * Guarantees PAGE-ALIGNED buffers in ALL cases (true XRT BO or posix_memalign fallback)
 * Pre/post-processing 100% identical to official main.cc
 */
...
static bool init_xrt_io_buffers(vart::RunnerExt* runner) {
    ...
    // Input TensorBuffer
    {
        ...
        printf("[XRT][Input]  vaddr=0x%016lX  paddr=0x%016lX  size=%zu  %s\n",
               ...
               g_input_xrt.xrt_confirmed
                   ...
                   : "WARNING: NOT page-aligned -- using fallback");
    }
    ...
    g_output_xrt.vaddr = g_fallback_output;
    g_input_xrt.xrt_confirmed  = true;
    g_output_xrt.xrt_confirmed = true;
...
// =============================================================================
// HELPERS (100% identical to official main.cc)
// =============================================================================
...
// =============================================================================
// INFERENCE via XRT TensorBuffers (FIXED lifetime for fallback)
// =============================================================================
static void run_one_inference_xrt(vart::RunnerExt* runner,
    ...
    // === CRITICAL FIX: keep TensorBuffers alive for the whole function ===
    ...
    if (g_fallback_input != nullptr) {
        printf("[Fallback] Using manually page-aligned TensorBuffers\n");
        ...
        inputs_buf.push_back(std::make_unique<CpuFlatTensorBuffer>(
            g_fallback_input, batchTensors.back().get()));

        auto out_dims = outputs[0]->get_tensor()->get_shape();
        batchTensors.push_back(std::shared_ptr<xir::Tensor>(
            xir::Tensor::create(outputs[0]->get_tensor()->get_name(), out_dims,
                                xir::DataType{xir::DataType::XINT, 8u})));
        outputs_buf.push_back(std::make_unique<CpuFlatTensorBuffer>(
            g_fallback_output, batchTensors.back().get()));
