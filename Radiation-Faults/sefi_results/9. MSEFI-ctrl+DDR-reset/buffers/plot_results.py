#!/usr/bin/env python3
# SEFI Simulation Plots — mode: MSEFI-ctrl+DDR-reset, target: buffers
...
if os.path.exists(acc_path):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTDIR, 'plot_accuracy_MSEFI-ctrl+DDR-reset_buffers.png'), dpi=150)
    plt.close(); print('[Plot] plot_accuracy_MSEFI-ctrl+DDR-reset_buffers.png')
...
if os.path.exists(csv_path):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTDIR, 'plot_prob_drop_MSEFI-ctrl+DDR-reset_buffers.png'), dpi=150)
    plt.close(); print('[Plot] plot_prob_drop_MSEFI-ctrl+DDR-reset_buffers.png')
...
if os.path.exists(csv_path):
    ...
    if 'bytes_corrupted' in df.columns and df.bytes_corrupted.max() > 0:
        ...
        plt.xticks(rotation=45, ha='right', fontsize=8)
        plt.tight_layout()
        plt.savefig(os.path.join(OUTDIR, 'plot_bytes_corrupted_MSEFI-ctrl+DDR-reset_buffers.png'), dpi=150)
        plt.close(); print('[Plot] plot_bytes_corrupted_MSEFI-ctrl+DDR-reset_buffers.png')
