#!/usr/bin/env python3
# SEFI Simulation Plots — mode: MSEFI-ctrl-reset
...
if os.path.exists(acc_path):
    ...
    ax.set_title('SEFI Mode: MSEFI-ctrl-reset — Accuracy', fontsize=13)
    ax.legend(fontsize=10)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTDIR, 'plot_accuracy.png'), dpi=150)
    plt.close(); print('[Plot] plot_accuracy.png')

csv_path = os.path.join(OUTDIR, 'results_MSEFI-ctrl-reset.csv')
if os.path.exists(csv_path):
    ...
    ax.set_title('SEFI MSEFI-ctrl-reset — Prob Drop per Image', fontsize=12)
...
if os.path.exists(csv_path):
    ...
    if 'bytes_corrupted' in df.columns and df.bytes_corrupted.max() > 0:
        ...
        ax.set_title('SEFI MSEFI-ctrl-reset — Bytes Corrupted per Image', fontsize=12)
        plt.xticks(rotation=45, ha='right', fontsize=8)
        plt.tight_layout()
