#!/usr/bin/env python3
# SEFI Simulation Plots — mode: transient-SEFI-col
...
if os.path.exists(acc_path):
    ...
    vals   = [row.baseline_accuracy_pct, row.faulty_accuracy_pct]
    ...
    ax.set_title('SEFI Mode: transient-SEFI-col — Accuracy', fontsize=13)
    ax.legend(fontsize=10)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTDIR, 'plot_accuracy.png'), dpi=150)
    plt.close(); print('[Plot] plot_accuracy.png')

csv_path = os.path.join(OUTDIR, 'results_transient-SEFI-col.csv')
if os.path.exists(csv_path):
    ...
    ax.set_title('SEFI transient-SEFI-col — Prob Drop per Image', fontsize=12)
    ax.set_xlabel('Image'); ax.set_ylabel('Baseline prob − Faulty prob')
    plt.xticks(rotation=45, ha='right', fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(OUTDIR, 'plot_prob_drop.png'), dpi=150)
    plt.close(); print('[Plot] plot_prob_drop.png')
...
if os.path.exists(csv_path):
    ...
    if 'bytes_corrupted' in df.columns and df.bytes_corrupted.max() > 0:
        ...
        ax.set_title('SEFI transient-SEFI-col — Bytes Corrupted per Image', fontsize=12)
        plt.xticks(rotation=45, ha='right', fontsize=8)
        plt.tight_layout()
        plt.savefig(os.path.join(OUTDIR, 'plot_bytes_corrupted.png'), dpi=150)
        plt.close(); print('[Plot] plot_bytes_corrupted.png')
